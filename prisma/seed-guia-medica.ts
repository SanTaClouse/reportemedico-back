/**
 * Seed de la Guía Médica (V2) — catálogos cerrados (docs/v2/01 §4).
 *
 * ⚠️ DATOS PROVISIONALES: las listas definitivas (especialidades, ARS, ciudades
 * y clínicas con dirección exacta) las provee Alberto (docs/v2/00 §7.4).
 * Las coordenadas de clínicas son aproximadas — reemplazar con las exactas
 * (Google Maps → clic derecho → copiar coordenadas) antes del deploy.
 *
...
// null = sin match limpio en el enum; el JSON-LD omite el campo.

const specialties: { name: string; slug: string; schemaOrgValue: string | null }[] = [
...
const insurances: { name: string; slug: string }[] = [
...
// ─── Clínicas (coordenadas APROXIMADAS — reemplazar antes del deploy) ─────────

const clinics: { name: string; slug: string; citySlug: string; address: string; latitude: number; longitude: number; phone?: string }[] = [
  // Santo Domingo
...
// ─── Mapeo inicial tags (V1) ↔ especialidades ────────────────────────────────
// Solo se crean los cruces cuyos tags existan en la BD (slugs del seed V1).

const specialtyTagMap: Record<string, string[]> = {
...
async function main() {
  console.log('🌱 Seed Guía Médica (catálogos provisionales)...\n')
  ...
  for (const c of cities) {
    const city = await prisma.city.upsert({ where: { slug: c.slug }, update: { name: c.name }, create: c })
  ...
  console.log('⚠️  Recordatorio: reemplazar clínicas/coordenadas con la lista definitiva de Alberto.\n')
}
